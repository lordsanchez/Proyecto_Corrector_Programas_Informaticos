...
int TemporalAnalisisOfStudentSample(int idProblem) {
	...
	for (i = 0; i < numberOfTests; i++) {
		...
		if (ret == 0) { //child
			...
			if (time_spent <= pubicsTimesPofessorTest[i]*limit) {
	...
	for (i = 0; i < numberOfTests; i++) {
		...
		if (ret == 0) { //child
			...
			gettimeofday(&end, NULL);
			time_spent=(end.tv_sec - begin.tv_sec)*1000 + (end.tv_usec - begin.tv_usec)/1000.0;
			if (time_spent <= privatesTimesPofessorTest[i]*limit) {
...
int main (int argc, char *argv[])
{
    ...
    while(1) {
		...
		if(ret > 0) { /* the father process, wait until child compiles and response */
			if (compiler == 0 || compiler == 1) { //compiladors c, cpp
				wait(&status);	
				if(status == 0) {
					if (needPassAll) { 
					...
					if (testsPassed == -1) { //some diff test had failed
					...
					} else { //the diff test has been passed, proced temporal analisi
						ret=fork();
						if(ret > 0) { /* the father process, wait until child compiles and response */
							if (compilerSample == 0 || compilerSample == 1) { //compiladors c, cpp
								...
								if(status == 0) {
									...
									int result = TemporalAnalisisOfStudentSample(idProblem);
									int fd5 = open("result.txt", O_RDWR | O_APPEND | O_CREAT , 0644);
									if (fd5 < 0) {
										perror("Error on open 13\n");
										//exit(EXIT_FAILURE);
						...
						} else if (ret == 0) {// the first child process, compile the program
			...
			CompileProblem(compiler, 0, idProblem);
